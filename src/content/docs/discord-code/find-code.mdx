...
What this means is that the [module](/discord-code/modules#modules) this code is in was patched by a plugin. Whenever a module is patched, it is extracted into its own file called `WebpackModuleXXXXXX`.

So the first result is the actual module in use. The second result is the [chunk](/discord-code/modules#chunks) this module was contained in. However the module in this chunk is no longer used.
:::
...
You will notice that this is only a small [module](/discord-code/modules#modules) which exports one or more css classes. The export names may be short one or two letter names.
...
If you are confused, see the [modules](/discord-code/modules#modules) documentation for more information.
